...
namespace API.Data
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<string> GetUserMentorOrMenteeStatus(string username)
        {
            return await _context.Users
                .Where(x => x.UserName == username)
                .Select(x => x.MentorOrMentee)
                .FirstOrDefaultAsync();
        }
        ...
        {
            return await _context.Users
                .Include(p => p.Photos) // eager-loading the entity
                .ToListAsync();
        }

        // public async Task<bool> SaveAllAsync()
        // {
        //     return await _context.SaveChangesAsync() > 0; // will return how many changes were made to the db
        // }

        public void Update(AppUser user)
        {
            _context.Entry(user).State = EntityState.Modified; // informs the EF tracker that an entity has been updated
        }
    }
}
